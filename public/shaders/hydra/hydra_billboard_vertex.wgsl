...
fn vs_main(@builtin(vertex_index) vertexIndex: u32) -> VertexOutput {
    ...
    // В WGSL нельзя писать clipPos.xy += ..., поэтому собираем vec4 заново.
    let offset = corner * uniforms.particleRadius * clipPos.w;

    clipPos = vec4f(clipPos.x + offset.x, clipPos.y + offset.y, clipPos.z, clipPos.w);
